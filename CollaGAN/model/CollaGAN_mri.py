...
from model.netUtilmri import  Conv2d, Conv2d2x2, lReLU, BN, Conv1x1, UnetINDiv4_CCAM
...
class CollaGAN:
    def __init__(self, opt):
        ...
        self.scale = 45.0 # if opt.wo_norm_std else 25.0 % temporaly for display   ##255.0
        ...
        input_contrasts = tf.concat([inp1,inp2,inp3,inp4,inp5,inp6],axis=ch_dim) 
        self.inputs = tf.concat([input_contrasts, self.a_mask, self.b_mask,self.c_mask,self.d_mask,self.e_mask,self.f_mask],axis=ch_dim)
        ...
        cyc_inp1_ = tf.concat([tmp_zeros,cyc2_,cyc3_,cyc4_,cyc5_,cyc6_],axis=ch_dim)
        cyc_inp2_ = tf.concat([cyc1_,tmp_zeros,cyc3_,cyc4_,cyc5_,cyc6_],axis=ch_dim)
        cyc_inp3_ = tf.concat([cyc1_,cyc2_,tmp_zeros,cyc4_,cyc5_,cyc6_],axis=ch_dim)
        cyc_inp4_ = tf.concat([cyc1_,cyc2_,cyc3_,tmp_zeros,cyc5_,cyc6_],axis=ch_dim)
        cyc_inp5_ = tf.concat([cyc1_,cyc2_,cyc3_,cyc4_,tmp_zeros,cyc6_],axis=ch_dim)
        cyc_inp6_ = tf.concat([cyc1_,cyc2_,cyc3_,cyc4_,cyc5_,tmp_zeros],axis=ch_dim)

        atmp_zeros = tf.zeros([self.nB,1,self.nY,self.nX],dtype)
        atmp_ones  = tf.ones([self.nB,1,self.nY,self.nX],dtype)
        cyc_inp1 = tf.concat([cyc_inp1_,atmp_ones,atmp_zeros,atmp_zeros,atmp_zeros,atmp_zeros,atmp_zeros],axis=ch_dim)
        cyc_inp2 = tf.concat([cyc_inp2_,atmp_zeros,atmp_ones,atmp_zeros,atmp_zeros,atmp_zeros,atmp_zeros],axis=ch_dim)
        cyc_inp3 = tf.concat([cyc_inp3_,atmp_zeros,atmp_zeros,atmp_ones,atmp_zeros,atmp_zeros,atmp_zeros],axis=ch_dim)
        cyc_inp4 = tf.concat([cyc_inp4_,atmp_zeros,atmp_zeros,atmp_zeros,atmp_ones,atmp_zeros,atmp_zeros],axis=ch_dim)
        cyc_inp5 = tf.concat([cyc_inp5_,atmp_zeros,atmp_zeros,atmp_zeros,atmp_zeros,atmp_ones,atmp_zeros],axis=ch_dim)
        cyc_inp6 = tf.concat([cyc_inp6_,atmp_zeros,atmp_zeros,atmp_zeros,atmp_zeros,atmp_zeros,atmp_ones],axis=ch_dim)
        ...
        G_gan_loss_cyc  = G_gan_loss_cyc1 + G_gan_loss_cyc2 + G_gan_loss_cyc3 + G_gan_loss_cyc4 + G_gan_loss_cyc5 + G_gan_loss_cyc6 
        ...
        l2_cyc_loss = cyc_l2_loss1 + cyc_l2_loss2 + cyc_l2_loss3 + cyc_l2_loss4 + cyc_l2_loss5 + cyc_l2_loss6

        l2_loss_orig = tf.reduce_mean(tf.squared_difference(self.recon,self.targets))
        l2_loss = (l2_loss_orig+l2_cyc_loss)/7.
        ...
        l1_cyc_loss = cyc_l1_loss1 + cyc_l1_loss2 + cyc_l1_loss3 + cyc_l1_loss4 + cyc_l1_loss5  + cyc_l1_loss6

        l1_loss_orig = tf.reduce_mean(tf.losses.absolute_difference(self.recon,self.targets))
        l1_loss = (l1_loss_orig+l1_cyc_loss)/7.
        ...
        ssim5= tf.image.ssim(self.cyc5[0,0,:,:,tf.newaxis], self.e_img[0,0,:,:,tf.newaxis], 5)
        ssim6= tf.image.ssim(self.cyc6[0,0,:,:,tf.newaxis], self.f_img[0,0,:,:,tf.newaxis], 5)
        ...
        cyc_ssim_loss5 = -tf.log( (1.0+ssim5)/2.0) 
        cyc_ssim_loss6 = -tf.log( (1.0+ssim6)/2.0)  

        cyc_ssim_loss1 = tf.cond(self.bool1, lambda:0., lambda: -tf.log( (1.0+ssim1)/2.0))
        cyc_ssim_loss2 = tf.cond(self.bool2, lambda:0., lambda: -tf.log( (1.0+ssim2)/2.0))
        cyc_ssim_loss3 = tf.cond(self.bool3, lambda:0., lambda: -tf.log( (1.0+ssim3)/2.0))
        cyc_ssim_loss4 = tf.cond(self.bool4, lambda:0., lambda: -tf.log( (1.0+ssim4)/2.0))
        cyc_ssim_loss5 = tf.cond(self.bool5, lambda:0., lambda: -tf.log( (1.0+ssim5)/2.0))
        cyc_ssim_loss6 = tf.cond(self.bool6, lambda:0., lambda: -tf.log( (1.0+ssim6)/2.0))
        ssim_cyc_loss  = cyc_ssim_loss1 + cyc_ssim_loss2 + cyc_ssim_loss3 + cyc_ssim_loss4 + cyc_ssim_loss5 + cyc_ssim_loss6 
#
        ssim_loss_orig = -tf.log( (1.0+ssimr)/2.0)   
        ssim_loss = (ssim_loss_orig + ssim_cyc_loss)/7.
        ...
        G_clsf_cyc_loss  = G_clsf_cyc_loss1 + G_clsf_cyc_loss2 + G_clsf_cyc_loss3 + G_clsf_cyc_loss4 + G_clsf_cyc_loss5  + G_clsf_cyc_loss6 
        G_clsf_orig_loss = tf.losses.softmax_cross_entropy(OH_labelT, type_rec)
        G_clsf_loss = (G_clsf_orig_loss + G_clsf_cyc_loss)/7.
        ...
        self.G_loss       = (self.lambda_GAN*(G_gan_loss_orig + G_gan_loss_cyc) +
                 ...
                 self.lambda_ssim_cyc* (ssim_cyc_loss)+ 2*ssim_loss_orig
                 )
        ...
        self.C_loss       = C_loss1 + C_loss2 + C_loss3 + C_loss4+ C_loss5 + C_loss6 
        ...
        D_gan_cyc  = cyc_err1 + cyc_err2 + cyc_err3 + cyc_err4 + cyc_err5  + cyc_err6
        D_gan_loss  = (D_err + D_gan_cyc)/7.
        ##
        self.D_loss = (D_err + D_gan_cyc)/7. + (self.C_loss)/6.
        ...
        tf.summary.image('1inputs/2flair', self.tf_vis( self.inputs[:,1,tf.newaxis,:,:] ) )
        tf.summary.image('1inputs/3T2w', self.tf_vis( self.inputs[:,2,tf.newaxis,:,:] ) )
        tf.summary.image('1inputs/4gm', self.tf_vis( self.inputs[:,3,tf.newaxis,:,:] ) )
        ...
        tf.summary.image('2cycle/2flair', cyc2_rgbv)
        tf.summary.image('2cycle/3T2star', cyc3_rgbv)
        tf.summary.image('2cycle/4gm', cyc4_rgbv)
        ...
        self.reconpic=self.recon
        self.ssim= tf.image.ssim(self.recon[0,0,:,:,tf.newaxis], self.targets[0,0,:,:,tf.newaxis], 5)
        self.mse = tf.losses.mean_squared_error(self.recon[0,0,:,:,tf.newaxis], self.targets[0,0,:,:,tf.newaxis])
        self.psnr = tf.image.psnr(self.recon[0,0,:,:,tf.newaxis], self.targets[0,0,:,:,tf.newaxis], max_val=(tf.reduce_max(self.targets[0,0,:,:,tf.newaxis])-tf.reduce_min(self.targets[0,0,:,:,tf.newaxis])))
...
class Discriminator:
    def __init__(self, name='D', nCh=4, w_decay_D=0,DR_ratio=0, class_N=6): #class=4
        self.name   = name
    ...
    def __call__(self, input, is_Training):
        ...
        self.reuse=True
        self.variables = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=self.name)
        return RF_out, Class_out[:,tf.newaxis,:,0,0] #Class_out[tf.newaxis,:,:,0,0]